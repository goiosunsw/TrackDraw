...
# Generate parameters
DEFAULT_PARAMS = TDD.Parameters()
CURRENT_PARAMS = TDD.Parameters()
...
class Slots:
    ...
    def audioOpen(self, *arg, parent=None, **kwarg):
        ...
        if fname[0]:
            ...
            new_fs = DEFAULT_PARAMS.resample_fs
            new_n  = round(new_fs/old_fs*len(x))
            new_x  = signal.resample(x, new_n)
            LOADED_SOUND.waveform = new_x
            LOADED_SOUND.fs = new_fs
        if self.master.displayDock.loadedRadioButton.isChecked():
            self.master.cw.spec_cv.plot_specgram(LOADED_SOUND.dur,
                                                 LOADED_SOUND.waveform,
                                                 LOADED_SOUND.fs,
                                                 CURRENT_PARAMS.window_len,
                                                 CURRENT_PARAMS.noverlap,
                                                 CURRENT_PARAMS.window_type,
                                                 TRACKS)
            self.master.cw.wave_cv.plot_waveform(LOADED_SOUND.waveform)
    ...
    def applyDefaults(self, *arg, **kwarg):
        ...
        self.master.cw.f0_cv.start(F0_TRACK)
        # Reset sliders
    ...
    def enableBubble(self, *arg, **kwarg):
        if self.master.displayDock.trackBubbleCheckBox.isChecked():
            CURRENT_PARAMS.track_bubble = True
        elif self.master.displayDock.trackBubbleCheckBox.isChecked():
            CURRENT_PARAMS.track_bubble = False

    @pyqtSlot()
    def changeTracks(self, *arg, **kwarg):
        CURRENT_PARAMS.bubble_len = self.master.displayDock.trackGroup.sliders["Bubble size"].value()
        new_track_npoints = self.master.displayDock.trackGroup.sliders["Number of points"].value()
        if new_track_npoints != CURRENT_PARAMS.track_npoints:
            CURRENT_PARAMS.track_npoints = new_track_npoints
            self.master.cw.spec_cv.track_npoints = new_track_npoints
            self.master.cw.f0_cv.track_npoints = new_track_npoints
            for i in range(len(TRACKS)):
                TRACKS[i].changeNoPoints(new_track_npoints)
            F0_TRACK[0].changeNoPoints(new_track_npoints)
            waveform, fs, dur = self.getCurrentWaveform()
            self.pushDisplayUpdates(waveform, fs, dur)
    ...
    def applyAnalysis(self, *arg, **kwarg):
        ...
        """
        self.master.cw.spec_cv.plot_specgram(window_len=CURRENT_PARAMS.window_len,
                                             window_type=CURRENT_PARAMS.window_type,
                                             noverlap=CURRENT_PARAMS.noverlap,
                                             tracks=TRACKS, restart=True)
    ...
    def changeWindow(self, curr_index, *arg, **kwarg):
        ...
        """
        if curr_index == 0:
            CURRENT_PARAMS.window_type = np.hamming
        if curr_index == 1:
            CURRENT_PARAMS.window_type = np.bartlett
        if curr_index == 2:
            CURRENT_PARAMS.window_type = np.blackman
    ...
    def changeSynth(self, curr_index, *arg, **kwarg):
        ...
        """
        if curr_index == 0:
            CURRENT_PARAMS.synth_type = "Klatt 1980"
        if curr_index == 1:
            CURRENT_PARAMS.synth_type = "Sine wave"
    ...
    def synthesize(self, *arg, **kwarg):
        ...
        if self.master.displayDock.synthedRadioButton.isChecked():
            self.master.cw.spec_cv.plot_specgram(x_right=SYNTH_SOUND.dur,
                                                 ...
                                                 fs=SYNTH_SOUND.fs,
                                                 window_len=CURRENT_PARAMS.window_len,
                                                 noverlap=CURRENT_PARAMS.noverlap,
                                                 window_type=CURRENT_PARAMS.window_type,
                                                 tracks=TRACKS)
    ...
    def mouse(self, *arg, **kwarg):
        ...
        if event.button and x_loc != None:
            ...
            dist_to_x_pts = np.abs(np.linspace(0,
                                   CURRENT_PARAMS.track_npoints-1,
                                   CURRENT_PARAMS.track_npoints)-x_loc)
            ...
            if CURRENT_PARAMS.track_bubble == True and len(tracks) > 1:
                if y_coords_at_nearest_x[plot.locked_track] < y_loc:
                    try:
                        ...
                        if difference <= CURRENT_PARAMS.bubble_len:
                            tracks[plot.locked_track].points[nearest_x_idx] =\
                                    val_above - CURRENT_PARAMS.bubble_len
                        else:
                ...
                if y_coords_at_nearest_x[plot.locked_track] > y_loc:
                    try:
                        ...
                        if difference <= CURRENT_PARAMS.bubble_len:
                            tracks[plot.locked_track].points[nearest_x_idx] =\
                                    val_below + CURRENT_PARAMS.bubble_len
                        else:
                            tracks[plot.locked_track].points[nearest_x_idx] = y_loc
                    except IndexError:
                        tracks[plot.locked_track].points[nearest_x_idx] = y_loc
        ...
        if self.master.cw.stft_cv.enabled == True:
            waveform, fs, dur = self.getCurrentWaveform()
            try:
                x_loc = x_loc/CURRENT_PARAMS.track_npoints
                x_loc = int(x_loc*dur*fs)
                if CURRENT_PARAMS.stft_size < x_loc < round(fs*dur)\
                        -CURRENT_PARAMS.stft_size:
                    waveform = waveform/np.max(np.abs(waveform))
                    magnitude = np.fft.rfft(waveform[x_loc-CURRENT_PARAMS.stft_size:x_loc
                                                     +CURRENT_PARAMS.stft_size])
    ...
    def mouse_ctrl(self, *arg, **kwarg):
        ...
        if event.button:
            try:
                ...
                wasClick = kwarg["wasClick"]
                x_loc, y_loc = plot.mouse(event)
                dist_to_x_pts = np.abs(np.linspace(0,CURRENT_PARAMS.track_npoints-1,CURRENT_PARAMS.track_npoints) - x_loc)
                nearest_x_idx = dist_to_x_pts.argmin()
                if target == "F0":
                    if wasClick == True:
                        plot.locked_point = nearest_x_idx
                        F0_TRACK[0].points[nearest_x_idx] = y_loc
                        plot.update_track(F0_TRACK[0].points)
                    elif wasClick == False:
                        if plot.locked_point == nearest_x_idx:
                            F0_TRACK.points[nearest_x_idx] = y_loc
                            plot.update_track(F0_TRACK.points)
                        elif plot.locked_point != nearest_x_idx:
                            difference = abs(plot.locked_point - nearest_x_idx)
                            if plot.locked_point > nearest_x_idx:
                                F0_TRACK.points[nearest_x_idx:plot.locked_point] = np.linspace(y_loc, F0_TRACK.points[plot.locked_point], difference)
                            elif plot.locked_point < nearest_x_idx:
                                F0_TRACK.points[plot.locked_point:nearest_x_idx] = np.linspace(F0_TRACK.points[plot.locked_point], y_loc, difference)
                            plot.update_track(F0_TRACK.points)
                elif target == "FF":
                    if wasClick == True:
                        y_coords_at_nearest_x = np.array([track.points[nearest_x_idx] for track in TRACKS])
                        dist_to_y_pts = np.abs(y_coords_at_nearest_x - y_loc)
                        trackNo = dist_to_y_pts.argmin()
                        TRACKS[trackNo].points[nearest_x_idx] = y_loc
                        plot.update_track(TRACKS[trackNo].points, trackNo)
                        plot.locked_track = trackNo
                    elif wasClick == False:
                        TRACKS[plot.locked_track].points[nearest_x_idx] = y_loc
                        plot.update_track(TRACKS[plot.locked_track].points, plot.locked_track)
            except TypeError:
                pass
        else:
            try:
        ...
        try:
            x_loc = x_loc/CURRENT_PARAMS.track_npoints
            x_loc = int(x_loc*dur*fs)
            if CURRENT_PARAMS.stft_size < x_loc < round(fs*dur)-CURRENT_PARAMS.stft_size:
                waveform = waveform/np.max(np.abs(waveform))
                magnitude = np.fft.rfft(waveform[x_loc-CURRENT_PARAMS.stft_size:x_loc+CURRENT_PARAMS.stft_size])
                magnitude = 20*np.log10(np.abs(magnitude))
                self.master.cw.stft_cv.update_stft(magnitude)
        except TypeError:
            pass
    ...
    def getCurrentWaveform(self):
        ...
        if self.master.displayDock.synthedRadioButton.isChecked():
            ...
            waveform = LOADED_SOUND.waveform
            fs = LOADED_SOUND.fs
            dur = LOADED_SOUND.dur
    ...
    def pushDisplayUpdates(self, waveform, fs, dur):
        ...
        if len(waveform) == 1:
            ...
            self.master.cw.spec_cv.plot_specgram(dur, waveform, fs,
                    CURRENT_PARAMS.window_len,
                    CURRENT_PARAMS.noverlap,
                    CURRENT_PARAMS.window_type, TRACKS)
            self.master.cw.wave_cv.plot_waveform(waveform)
            self.master.cw.f0_cv.start(F0_TRACK)
